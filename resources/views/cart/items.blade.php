                            ...
                            <table class="w-full text-md bg-white shadow-md rounded mb-4 ">
                                ...
                                @foreach($items as $item)
                                    <tr class="border-b hover:bg-orange-100 bg-gray-100">
                                        <td class="p-5" >
                                            <div
                                                ...
                                                @if($item['book']->images()->count() > 0 )
                                                    <img src="{{ asset('storage/' . $item['book']->images()->first()->file_path)}}"
                                                         alt=" Post image "
                                        ...
                                        <td class="p-5">
                                            <input
                                                class="p-1 w-1/12 rounded-lg text-center"
                                                value="{{$item['quantity']}}">
                                        ...
                                        <td class="p-5"  >
                                        <form method="POST" action="{{route('cart.destroy',['id'=>$item['book']->id])}}">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
